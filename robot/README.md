...
外形は 幅 210（2 階の天板）× 奥行き 304（1 階天板の前端〜本棚の後端）。実機のホイール外側どうしは 215 mm（トレッド 175 ＋ 幅 40）で、奥行き方向にはホイールははみ出しません。

| 測ったもの | 値 |
|---|---|
| メカナムの土台（モータが付くシャーシの板） | 幅 155 × 長さ 200 mm（実機の採寸と CAD が一致）。1 階天板（155 × 300）はその上に 15 mm のスペーサで載り、前後に約 50 mm ずつ張り出す |
| 前後の車軸の中間（＝回転の中心）の位置 | 土台の板の真ん中 ＝ 1 階天板の前端（基板側）から 約 150 mm（天板のほぼ真ん中） |
| いちばん遠い角 | 2 階天板の前の角まで 184 mm（本棚の後ろの角は 173 mm） |
| 指定値（＋2 cm） | **`0.21`**（通路は最低 45 cm ほど必要） |

CAD のホイールベースは 105 mm、実機は 95 mm（9/20 に測り直して確認）で 1 cm 食い違っています。どちらのモータがずれているか分からないので回転の中心は前後に最大 5 mm ずれ得ますが、そのときでも最遠の角は 188 mm で `0.21` に収まります。
本棚や天板の形を変えたら測り直してください（中心が前端から 100 mm まで前に寄ると、本棚の後ろの角が 218 mm になり `0.24` が必要）。

**LiDAR の取付位置（同じ計測。`robot_tf_launch.py` / `temp_tf_launch.py` に入れる値の目安）**

LiDAR は本棚の上（機体でいちばん高い位置）にあり、回転の中心から **後ろ（本棚側）へ 約 86 mm・左右のずれ 0**、スキャン面は床から 約 32 cm です。
基板側を「前」とするなら `laser_x:=-0.086 laser_y:=0.0 laser_z:=0.32`（既定値の `laser_x:=0.10` とは向きも量も違います）。
★要確認★ 前進の指令で実機が本棚側へ進むなら `laser_x` は ＋0.086 です。LiDAR の 0° の向き（`laser_yaw`）は CAD からは決められないので、RViz で壁の向きを見て確かめます。地図づくりと実走で同じ値を使うこと。
