...
ホイールベースは、実機の定規での読みが 95 mm（軸穴の縁どうしを読んだ可能性があり、中心間は 95〜100 mm）、CAD はハブ穴の中心間 100 mm・モータ軸の間隔 105 mm で、まだ 1 つに決まっていません（`robot_params.py` のコメント参照。旋回の較正で合わせ込みます）。どの値でも前後の車軸は土台の板の真ん中に対してほぼ対称で、回転の中心が前後に 5 mm ずれたとしても最遠の角は 188 mm なので、半径は `0.21` のままです。
